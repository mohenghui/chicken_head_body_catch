...
from sensor_xy import DBD_RUN_XY
from onnx_detect_class import OnnxDetector
from sensor_z import DBD_RUN_Z
...
class MainWindow(QMainWindow):
    ...
    def init_model(self):
        ...
        if self.use_gpu:
            self.model=Detect_app(weight="best_head.pt",device="0",half=True,augment=False,conf_thres=0.25,iou_thres=0.45,classes=None,agnostic_nms=False,img_size=640)
        else:
            self.model=OnnxDetector(weight="best_head.onnx")
    def out_serial(self):
    ...
    def init_pic_label(self, text):
        ...
        self.label.setFixedSize(1080, 640)
    ...
    def get_frame_g_mutil(self):
        while self.g_open:
            ...
            if not self.g_open:
                break
            if self.start_predict:
                results = self.predict(self.img)
                if results:
                    for result in results:
                        # print(self.model.h,self.model.w,self.get_img.shape[0], self.get_img.shape[1])
                        new_result = scale_coords_new((self.img.shape[0],self.img.shape[1]), result[:4], (self.get_img.shape[0], self.get_img.shape[1]))
                        self.predict_lable.setText(
                            "预测的坐标为：{},类别为：{}".format(str([(new_result[2]+new_result[0])//2,(new_result[3]+new_result[1])//2]), 
                            str(self.classes[result[4]])))
                        

                        # new_result = scale_coords_new((self.model.h,self.model.w), result[:4], (self.get_img.shape[0], self.get_img.shape[1]))
                        # cls=result[4]
                        # if cls!=self.detect_obejct and self.detect_obejct!="all" and self.detect_obejct:return
                        # plot_one_box(new_result[:4], self.get_img, label=self.classes[cls], color=self.colors[int(cls)], line_thickness=1)

                        self.get_img=cv2.rectangle(self.get_img,(new_result[0],new_result[1]-1),(new_result[2],new_result[3]-1),(0,255,0))
                        showImage = QtGui.QImage(
                            self.get_img.data, self.get_img.shape[1], self.get_img.shape[0], QtGui.QImage.Format_RGB888)
                        self.label.setPixmap(QtGui.QPixmap.fromImage(showImage))
                        # print((new_result[2]+new_result[0])//2//13.618)
                        self.run1.backward((result[2]+result[0])//2//13.618)
                        self.run2.forward(14)
                        self.run2.forward(0)
                        #13.618
    ...
    def close_all(self):
        ...
        self.show_timeout_open=False
        # self.send_open=False
        if self.run1:self.run1.stop()
        if self.run2:self.run2.stop()
        if self.send_open:
            self.close_result()
        if self.t1:
            self.t1.join()
